...
SmoothnessConstraint::SmoothnessConstraint(const double* frequencies, size_t n, double bandwidthHz) :
  ...
  size_t nChannelBlocks)
{
  ...
  for(size_t i=0; i!=nthreads; ++i)
    _fitData.emplace_back(_frequencies.data(), _frequencies.size(), _bandwidth);
}

std::vector<Constraint::Result> SmoothnessConstraint::Apply(
    std::vector<std::vector<dcomplex> >& solutions, double, std::ostream* statStream)
{
